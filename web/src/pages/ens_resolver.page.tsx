...
const EnsResolverPage = () => {
  ...
  const resolveENS = async () => {
    ...
    try {
      const response = await fetch(`${backend_url}/user_ens/${trimmedAddress}?useEns=true`, {
        method: 'GET',
        headers: {
          'nonce': session.nonce,
      ...
      if (response.ok && data.success) {
        setEnsName(data.ens || 'No ENS name found for this address');
      } else {
        setEnsName('No ENS name found for this address');
      }
    } catch (err) {
      console.error('ENS resolution error:', err);
      setError('Failed to resolve ENS name. The service may be unavailable.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-2xl">
        <div className="bg-white rounded-2xl p-1 md:p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Enter an Ethereum address to find its ENS name</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {loading && (
              <div className="text-center py-8">
                ...
                <p className="mt-4 text-gray-600">Resolving ENS name...</p>
              </div>
            )}
            ...
            {ensName && !loading && (
              <div className="bg-gradient-to-r from-orange-50 to-orange-50 border-l-4 border-orange-500 p-6 rounded-lg">
                <p className="text-sm text-gray-600 mb-2">ENS Name:</p>
