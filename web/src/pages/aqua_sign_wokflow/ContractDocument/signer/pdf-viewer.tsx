...
const PdfViewer: React.FC<PdfViewerProps> = ({
    ...
    useEffect(() => {
        if (typeof window !== 'undefined' && window.pdfjsLib) {
            ...
            console.warn(
                'PDF.js library (window.pdfjsLib) not found on component mount.'
            )
            setPdfLoadingError(
                'PDF viewer library failed to load. Please try refreshing the page.'
            )
        }
    }, [])
    ...
    useEffect(() => {
        if (!file || !isPdfjsLibLoaded) {
            ...
            if (
                ...
                (file &&
                    !isPdfjsLibLoaded &&
                    !pdfLoadingError?.includes('library failed to load'))
            ) {
                setPdfLoadingError(null)
            }
            if (file && !isPdfjsLibLoaded && !pdfLoadingError) {
                setPdfLoadingError(
                    'PDF viewer library is not ready. Waiting for it to load...'
                )
            }
            return
        }
        ...
        reader.onerror = error => {
            ...
            setPdfLoadingError(
                "Error reading file. Please ensure it's a valid PDF."
            )
        }
    ...
    const renderPage = useCallback(async () => {
        if (
            ...
            if (
                ...
                setPdfLoadingError(
                    `Cannot render page ${currentPage}: page number is out of range (1-${numPages}).`
                )
            } else if (!pdfDoc && file && isPdfjsLibLoaded) {
                // This state is handled by the useEffect for PDF loading
            }
            if (!pdfDoc && !pdfLoadingError)
                setPageDimensions({ width: 0, height: 0 }) // Clear dimensions if no doc and no explicit error
            return
        }
        ...
        try {
            ...
            if (!context) {
                ...
                setPdfLoadingError(
                    'Failed to initialize canvas for PDF rendering.'
                )
                setPageDimensions({ width: 0, height: 0 })
                return
            }
            ...
            setPdfLoadingError(
                `Error rendering page ${currentPage}: ${error instanceof Error ? error.message : String(error)}`
            )
            setPageDimensions({ width: 0, height: 0 })
            renderTaskRef.current = null
        }
    ...
    const handleAnnotationMouseDown = useCallback(
        (event: React.MouseEvent, annotation: Annotation) => {
            ...
            const clickXCanvasPercent =
                ((event.clientX - canvasRect.left) / canvasRect.width) * 100
            const clickYCanvasPercent =
    ...
    const handleDragMove = useCallback(
        (event: MouseEvent) => {
            ...
            let newX =
                (mouseXOnCanvas / canvasRect.width) * 100 - dragStartOffset.x
            let newY =
                (mouseYOnCanvas / canvasRect.height) * 100 - dragStartOffset.y
    ...
    const handleResizeMouseDown = useCallback(
        (
            ...
            const annotationElement = viewerRef.current?.querySelector(
                `[data-annotation-id="${elementToMeasureId}"]`
            ) as HTMLElement
            ...
            const initialElementXPercent = annotation.x
            const initialElementYPercent = annotation.y
    ...
    const handleResizeMouseMove = useCallback(
        (event: MouseEvent) => {
            ...
            if (handle.includes('Left')) {
                newPixelWidth = initialPixelWidth - deltaX
                const dxPercentChange =
                    (deltaX / initialPageDimensions.width) * 100
            ...
            if (handle.includes('top')) {
                newPixelHeight = initialPixelHeight - deltaY
                const dyPercentChange =
                    (deltaY / initialPageDimensions.height) * 100
            ...
            newBlockXPercent = Math.max(
                0,
                Math.min(newBlockXPercent, 100 - currentWidthPercentNum)
            )
            newBlockYPercent = Math.max(
                0,
                Math.min(
                    newBlockYPercent,
                    100 - parseFloat(finalHeightPercentString)
                )
            )
            ...
            if (currentAnnotation.type === 'image') {
                ...
                currentAnnotation.type === 'profile' &&
                targetPropertyPrefix === 'image'
            ) {
                updatedAnno = {
                    ...currentAnnotation,
    ...
    const handleViewerClick = (event: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (
            (target === viewerRef.current || target === canvasRef.current) &&
            !selectedTool &&
    ...
    const annotationsInDocumentOnCurrentPage = annotationsInDocument.filter(
    ...
    let annotationsOnCurrentPage = annotations.filter(
        anno => anno.page === currentPage
    )

    if (annotationsInDocumentOnCurrentPage.length > 1) {
        annotationsOnCurrentPage = annotationsOnCurrentPage.filter(
            e =>
                !annotationsInDocumentOnCurrentPage.some(
                    exist => exist.id == e.id
                )
        )
    }
    const renderResizeHandle = (
        ...
        switch (handleType) {
            case 'topLeft':
                ...
                positionStyle = { top: '-5px', left: '-5px' }
                break
            case 'topRight':
                cursorStyle = 'nesw-resize'
                positionStyle = { top: '-5px', right: '-5px' }
                break
            case 'bottomLeft':
                cursorStyle = 'nesw-resize'
                positionStyle = { bottom: '-5px', left: '-5px' }
                break
            case 'bottomRight':
                cursorStyle = 'nwse-resize'
                positionStyle = { bottom: '-5px', right: '-5px' }
                break
        }
    ...
    return (
        <div
            className="py-4 px-2 h-auto md:h-full"
            ref={viewerRef}
        ...
        >
            ...
            {pdfLoadingError && (
                <p className="text-center px-4">{pdfLoadingError}</p>
            )}
                    ...
                    {pdfDoc && (
                        <div className="relative flex flex-col justify-start overflow-x-auto">
                            ...
                            >
                                ...
                                />
                                {pageDimensions.width > 0 &&
                                    cleanSignatureToAvoidRepeating().map(
                                        anno => {
                                            const isSelected =
                                                selectedAnnotationId ===
                                                    anno.id ||
                                            ...
                                            if (anno.type === 'text') {
                                                ...
                                                return (
                                                    <div
                                                        key={anno.id}
                                                        style={profileStyle}
                                                        data-ai-hint="profile annotation"
                                                        data-annotation-id={
                                                            anno.id
                                                        }
                                                        onMouseDown={e =>
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <img
                                                                src={
                                                                    profileAnno.imageSrc
                                                                }
                                                                alt={
                                                                    profileAnno.imageAlt
                                                                }
                                                                style={{
                                                                    width: '100%',
                                                                    height: '100%',
                                                        ...
                                                        <div
                                                            style={{
                                                                fontSize:
                                                                    profileAnno.nameFontSize ||
                                                                    '12pt',
                                                                color:
                                                                    profileAnno.nameColor ||
                                                                    '#333333',
                                                                fontWeight:
                                                                    'bold',
                                                                pointerEvents:
                                                        ...
                                                        <div
                                                            style={{
                                                                fontSize:
                                                                    profileAnno.walletAddressFontSize ||
                                                                    '10pt',
                                                                color:
                                                                    profileAnno.walletAddressColor ||
                                                                    '#555555',
                                                                pointerEvents:
                                                                    'none',
                                                                wordBreak:
                                ...
                                {annotationsInDocumentOnCurrentPage.map(
                                    profileAnno => {
                                        const baseStyle: React.CSSProperties = {
                                            position: 'absolute',
                                            left: `${profileAnno.x}%`,
