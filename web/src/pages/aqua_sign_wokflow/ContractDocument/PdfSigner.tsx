...
const PdfSigner: React.FC<PdfSignerProps> = ({ fileData, setActiveStep, documentSignatures }) => {
    const { formTemplates, systemFileInfo, selectedFileInfo, setSelectedFileInfo, setFiles } =
        useStore(appStore);
    ...
    const linkMainDocumentWithSignatureData = async (
        ...
        const sigFileObject =
            getAquaTreeFileObject(selectedFileInfo!) ?? selectedFileInfo?.fileObject[0];
    ...
    const updateUIAfterSuccess = async () => {
        try {
            ...
            const selectedFile = files.find(
                data => getGenesisHash(data.aquaTree!) === selectedFileGenesisHash
            );
            ...
            for (const userSignature of userSignaturesApiInfo) {
                ...
                if (typeof fileContentUrl === 'string' && fileContentUrl.startsWith('http')) {
                    ...
                    const sign: SignatureData = {
                        ...
                        dataUrl: dataUrl,
                        createdAt:
                            timeStampToDateObject(firstRevision.local_timestamp) ?? new Date(),
        ...
        if (allSignersBeforeMe.length > 0) {
            return (
                <div className="flex flex-col gap-2 p-2 border border-gray-100 dark:border-gray-800 rounded-md">
                    <p className="text-md">
                        The following wallet address need to sign before you can.
                    </p>
                                ...
                                <div className="max-h-[200px] overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md">
                                    <div className="flex flex-col">
                                        {(() => {
                                            ...
                                            const signature = mySignatureData.find(
                                                sig =>
                                                    sig.hash === selectedSignatureId ||
        ...
        (newAnnotationData: Annotation) => {
            ...
            setSignaturePositions((prev: any) => {
                ...
                newData = newData.filter(
                    (item: SignatureData, index: number, self: SignatureData[]) =>
