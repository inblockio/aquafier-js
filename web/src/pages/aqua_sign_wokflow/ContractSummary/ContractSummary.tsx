...
export const ContractSummaryView: React.FC<ContractDocumentViewProps> = ({ setActiveStep, selectedFileInfo }) => {
      ...
      const isVerificationComplete = useCallback(
            (_verificationResults: VerificationHashAndResult[]): boolean => {
                  return selectedFileInfo?.aquaTree?.revisions ? _verificationResults.length === Object.keys(selectedFileInfo.aquaTree.revisions).length : false
            },
      ...
      const verifyAquaTreeRevisions = async (fileInfo: ApiFileInfo) => {
            ...
            try {
                  ...
                  const verificationPromises = revisionHashes.map(async revisionHash => {
                        ...
                        const result = await aquafier.verifyAquaTreeRevision(fileInfo.aquaTree!, revision, revisionHash, filesResult)
                        return {
                              hash: revisionHash,
                              ...
                              data={{
                                    ...
                                    signers: [
                                          ...firstRevisionData?.forms_signers.split(',').map((signer: string) => {
                                                const item = signatureRevionHashesData.find(e => e.walletAddress.toLowerCase().trim() == signer.toLowerCase().trim())
                                    ...
                                    activities: [
                                          {
                                                ...
                                                details: (
                                                      <>
                                                            Document <span className="font-bold">{fileNameData}</span> was selected for signing
                                                      </>
                                                ),
                                          },
                                          ...signatureRevionHashesData.map(_signatureRevionHasheItem => {
                                                ...
                                                return {
                                                      ...
                                                      timestamp:
                                                            singatureRevisionItem && singatureRevisionItem.local_timestamp
                                                                  ? timeToHumanFriendly(singatureRevisionItem.local_timestamp, true)
                                                                  : 'time   error',
