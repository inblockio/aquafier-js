...
const DNSClaim = ({ claimInfo }: IDNSClaim) => {
    ...
    const [verificationStatus, setVerificationStatus] = useState<
        'loading' | 'verified' | 'failed' | 'not_found' | 'pending'
    >('loading')
    ...
    const fields = Object.entries(claimInfo).map(([key, value]) => {
        ...
        return (
            <div key={key} className="flex justify-between items-start">
                ...
                <p
                    className={`text-sm font-medium max-w-[200px] break-all text-right ${cssClass}`}
                >
                    {processedValue}
                </p>
            </div>
    ...
    const getRecords = async () => {
        ...
        try {
            ...
            const domain =
                claimInfo['forms_domain'] || 'aqua._wallet.inblock.io'
            ...
            if (!currentWalletAddress || !currentSignature) {
                ...
                setVerificationMessage(
                    'Missing wallet address or signature in claim'
                )
                return false
            }
            ...
            for (const record of records) {
                try {
                    const { walletAddress, expiration } =
                        extractDNSClaimInfo(record)
                    ...
                    if (
                        ...
                        currentWalletAddress.trim().toLowerCase()
                    ) {
                        let verificationMsg =
                            'Signature verified via DNS TXT record'
                        if (expiration > 0) {
            ...
            setVerificationStatus('failed')
            setVerificationMessage(
                'No matching DNS record found for verification'
            )
            return false
        } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 max-w-md">
            ...
            {verificationStatus !== 'loading' && (
                ...
                >
                    ...
                    <p
                        className={`mt-1 text-xs ${verificationStatus === 'verified' ? 'text-green-700' : verificationStatus === 'not_found' ? 'text-amber-700' : 'text-red-700'}`}
                    >
                        {verificationMessage}
                    </p>
