...
const SignatureWalletAddressCard = ({
    walletAddress,
    signatureHash,
    index,
    timestamp,
}: ISignatureWalletAddressCard) => {
    ...
    const getWalletClaims = () => {
        ...
        if (files && files.length > 0) {
            let attestationFiles = files.filter(file => {
                ...
                return (
                    fileInfo.isWorkFlow &&
                    fileInfo.workFlow === 'identity_attestation'
                )
            })
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (aquaTree) {
                    ...
                    if (
                        isWorkFlow &&
                        (workFlow === 'identity_claim' ||
                            workFlow === 'domain_claim')
                    ) {
                        ...
                        if (walletAddress === _wallet_address) {
                            ...
                            for (let a = 0; a < attestationFiles.length; a++) {
                                ...
                                let attestationAquaTree =
                                    attestationFile?.aquaTree!
                                let attestationFileGenesisHash =
                            ...
                            let claimInformation: IClaim = {
                                ...
                                claimName:
                                    firstRevision.forms_name ??
                                    firstRevision.forms_domain,
    ...
    return (
        <div
            className={`${getClassesToRender()} flex flex-col gap-2 border-2 rounded-lg p-2 bg-white dark:bg-gray-800`}
        >
            <div
                className="flex align-start gap-2 p-4 cursor-pointer"
                onClick={openClaimPage}
            >
                {index && (
