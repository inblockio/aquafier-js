...
const CreateFormFromTemplate = ({
    ...
    const { session, backend_url, systemFileInfo, setSystemFileInfo, setFiles, selectedFileInfo } =
        useStore(appStore);
    ...
    const createWorkflowFromTemplate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const estimateize = estimateFileSize(JSON.stringify(completeFormData));
            ...
            if (genesisAquaTree.isOk()) {
                ...
                const containsFileData = selectedTemplate?.fields.filter(
                    e => e.type == 'file' || e.type == 'image' || e.type == 'document'
                );
                    ...
                    <div className="pt-10">
                        ...
                        >
                            ...
                            <div className="space-y-4 sm:space-y-6">
                                {selectedTemplate
                                    ? reorderInputFields(selectedTemplate.fields).map(
                                          (field, fieldIndex) => {
                                              const isFileInput =
                                                  field.type === 'file' ||
                                                  field.type === 'image' ||
