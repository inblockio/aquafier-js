...
export const DownloadAquaChain = ({
    file,
    index,
    children,
}: {
    ...
    const downloadLinkAquaJson = async () => {
        ...
        for (const fileObj of file.fileObject) {
            console.log(
                ...
                fileObj.fileName,
                'Content type:',
                typeof fileObj.fileContent
            )

            const isAquaTreeData = isAquaTree(fileObj.fileContent)
            if (
                typeof fileObj.fileContent === 'string' &&
                fileObj.fileContent.startsWith('http')
            ) {
                try {
                    const actualUrlToFetch = ensureDomainUrlHasSSL(
                        fileObj.fileContent
                    )
                    ...
                    console.error(
                        `Error downloading ${fileObj.fileName}:`,
                        error
                    )
                ...
                if (isAquaTreeData) {
                    ...
                    const jsonContent = JSON.stringify(
                        fileObj.fileContent as AquaTree
                    )
                    // Only add .json extension if it doesn't already have one
                    const fileName = fileObj.fileName.endsWith('.json')
                        ? fileObj.fileName
                        : `${fileObj.fileName}.json`
                    ...
                    fileObj.fileContent instanceof Uint8Array ||
                    fileObj.fileContent instanceof ArrayBuffer
                ) {
                    // Handle binary data
                    zip.file(fileObj.fileName, fileObj.fileContent, {
                        binary: true,
        ...
        for (const fileObj of file.fileObject) {
            ...
            if (
                ...
                try {
                    ...
                    if (
                        fileObj.fileContent instanceof Blob ||
                        fileObj.fileContent instanceof File
                    ) {
                        ...
                        if (isBinaryFile(fileObj.fileName)) {
                            ...
                            console.warn(
                                `Warning: ${fileObj.fileName} appears to be binary but stored as string`
                            )
                        }
                        blob = new Blob([fileObj.fileContent], {
                            type: 'text/plain',
                        ...
                        if (
                            isBinaryFile(fileObj.fileName) &&
                            isCorruptedBinaryData(fileObj.fileContent)
                        ) {
                            // Try to reconstruct binary data from object
                            const uint8Array = reconstructBinaryFromObject(
                                fileObj.fileContent
                            )
                            const mimeType = getMimeType(fileObj.fileName)
                        ...
                        throw new Error(
                            `Unsupported fileContent type for ${fileObj.fileName}`
                        )
                    }
            ...
            {children ? (
                ...
                >
                    {downloading ? (
                        <>
                            <svg
                                className="animate-spin h-3 w-3 mr-1 text-purple-700"
