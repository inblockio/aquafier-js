...
export const AttestAquaClaim = ({
        ...
        for (const anAquaTree of files) {
            ...
            if (isWorkFlow && isWorkFlow.workFlow == 'identity_attestation') {
                ...
                const genRevision: Revision = Object.values(
                    anAquaTree.aquaTree?.revisions!
                )[0]
                if (genRevision) {
                    const identityClaimId: string | undefined =
                        genRevision[`forms_identity_claim_id`]
                    console.log(
                        `identityClaimId  ${identityClaimId} genHash ${genHash} fileName ${getFileName(file.aquaTree!)}`
                    )
    ...
    if (secondRevision) {
        if (secondRevision.revision_type == 'signature') {
                ...
                return (
                    <div className="w-[100px]">
                        {children ? (
                            <div
                                data-testid={
                                    'attest-in-progress-aqua-claim-button-' +
                                    index
                                }
                                onClick={() => {
                                    if (!openCreateClaimAttestationPopUp) {
                                        ...
                                        toast(
                                            'Attesting is already in progress'
                                        )
                                    }
                                }}
                            >
                            ...
                            <button
                                data-testid={
                                    'attest-aqua-claim-button-' + index
                                }
                                onClick={() => {
                                    if (!openCreateClaimAttestationPopUp) {
                                        attestAquaClaimAction()
            ...
            toast.error(
                'This claim does not have a signature revision, cannot attest'
            )
            return null
        }
    }
