...
const WebsocketFragment = () => {
    ...
    const [websocketReconnectAttempts, setWebsocketReconnectAttempts] =
        useState(0)
    ...
    const fetchConnectedUsers = async () => {
        try {
            ...
            const users = response.data.clients.map(
    ...
    const connectWebsocket = () => {
        ...
        try {
            ...
            websocket.onopen = () => {
                ...
                pingInterval = setInterval(() => {
                    if (websocket.readyState === WebSocket.OPEN) {
                        websocket.send(
                            JSON.stringify({ action: 'ping', type: 'ping' })
                        )
                    }
                }, 20000)
            }
            ...
            websocket.onmessage = event => {
                try {
                    ...
                    if (message.action === WebSocketActions.REFETCH_FILES) {
                            ...
                            if (walletAddressRef.current && nounceRef.current) {
                                ...
                                if (currentSelectedFile) {
                                    ...
                                    const genesisHash = getGenesisHash(
                                        currentSelectedFile!.aquaTree!
                                    )
                                    if (genesisHash) {
                                        for (const itemTree of files) {
                                            const genesisHashItem =
                                                getGenesisHash(
                                                    itemTree.aquaTree!
                                                )
                                            if (genesisHashItem) {
                                        ...
                                        console.log(
                                            `🔌 - Genesis hash not found for selected file`
                                        )
                                    }
                                } else {
                                    ...
                                    console.log(
                                        `🔌 -3- No selected file ${currentSelectedFile}`
                                    )
                                }
                            } else {
                                console.log(
                                    `🔌 - Cannot refetch files as session or address or nounce is not defined DEBUG : ${JSON.stringify(session ?? {})}  `
                                )
                            }
                        })()
                    } else if (
                        message.action ===
                        WebSocketActions.REFETCH_SHARE_CONTRACTS
                    ) {
                        ;(async () => {
                            try {
                        ...
                        message.action === WebSocketActions.FETCH_USERS
                    ) {
                        fetchConnectedUsers()
                    } else {
            ...
            websocket.onclose = async event => {
                ...
                console.log(
                    `🔌 - Disconnected from WebSocket as user: ${isExplicitDisconnect}`
                )
