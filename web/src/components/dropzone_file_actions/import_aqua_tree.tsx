...
export const ImportAquaTree = ({ file, filesWrapper, removeFilesListForUpload}: IDropzoneAction) => {
      ...
      const findMissingFileForLinkHash = (aquaTreeItem: AquaTree, linkHash: string, newFileObjects: Array<{ file: File; fileObject: FileObject }>): UploadLinkAquaTreeExpectedData | null => {
                        ...
                        <DialogContent className="max-w-2xl w-[90%] mx-auto mt-12 rounded-2xl shadow-2xl bg-white border border-gray-200 p-6">
                              ...
                              <div className="py-4">
                                    <div className="flex flex-col items-center space-y-4">
                                          ...
                                          {selectedFileName.length > 0 && <p className="text-sm text-gray-600">Selected: {selectedFileName}</p>}
                                    </div>
                              </div>

                              <DialogFooter className="border-t border-gray-200 pt-3">
                                    <Button data-testid="action-cancel-77-button" variant="outline" onClick={closeModal} className="bg-black text-white hover:bg-gray-800">
                                          Cancel
                                    </Button>
                              </DialogFooter>
                        </DialogContent>
