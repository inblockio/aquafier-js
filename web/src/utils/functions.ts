...
export const isWorkFlowData = (
    ...
    if (secondRevision.revision_type == 'link') {
        ...
        if (name) {
            ...
            if (
                systemAndUserWorkFlow
                    .map(e => e.replace('.json', ''))
                    .includes(nameWithoutJson)
            ) {
                return {
                    isWorkFlow: true,
...
export function getAquaTreeFileObject(
...
export function validateAquaTree(tree: AquaTree): [boolean, string] {
    ...
    for (const hash in tree.revisions) {
        ...
        if (
            revision.local_timestamp === undefined ||
            revision.local_timestamp === null
        ) {
            return [false, 'A revision must contain local_timestamp ']
        }
        if (
            !revision.revision_type === undefined ||
            revision.local_timestamp === null
        ) {
        ...
        switch (revision.revision_type) {
            case 'file':
                if (
                    revision.file_hash === undefined ||
                    revision.file_hash === null
                ) {
                    return [false, 'file revision must contain file_hash']
                }
                if (
                    revision.file_nonce === undefined ||
                    revision.file_nonce === null
                ) {
            ...
            case 'witness':
                if (
                    ...
                    return [
                        false,
                        'witness revision must contain witness_merkle_root',
                    ]
                }
                if (
                    ...
                    return [
                        false,
                        'witness revision must contain witness_timestamp',
                    ]
                }
                if (
                    revision.witness_network === undefined ||
                    revision.witness_network === null
                ) {
                    return [
                        false,
                        'witness revision must contain witness_network',
                    ]
                }
                if (
                    ...
                    return [
                        false,
                        'witness revision must contain witness_smart_contract_address',
                    ]
                }
                if (
                    ...
                    return [
                        false,
                        'witness revision must contain witness_transaction_hash',
                    ]
                }
                if (
                    ...
                    return [
                        false,
                        'witness revision must contain witness_sender_account_address',
                    ]
                }
                break
            case 'signature':
                ...
                if (
                    ...
                    return [
                        false,
                        'signature revision must contain signature_public_key',
                    ]
                }
                if (
                    revision.signature_type === undefined ||
                    revision.signature_type === null
                ) {
                    return [
                        false,
                        'signature revision must contain signature_type',
                    ]
                }
                break
            case 'link':
                if (
                    revision.link_type === undefined ||
                    revision.link_type === null
                ) {
                ...
                if (
                    ...
                    return [
                        false,
                        'link revision must contain link_verification_hashes',
                    ]
                }
                if (!Array.isArray(revision.link_verification_hashes)) {
                    return [
                        false,
                        "link revision's link_verification_hashes must be an array",
                    ]
                }
...
export function timeToHumanFriendly(
    ...
    const resolvedTimezone =
        timezoneMap[defaultTimezone.toUpperCase()] || defaultTimezone
    ...
    try {
        ...
        return date.toLocaleDateString(
            'en-US',
            showFull ? fullOptions : dateOptions
        )
    } catch (error) {
        // Fallback to user's local timezone if specified timezone is invalid
        console.warn(
            `Invalid timezone: ${defaultTimezone}, falling back to local timezone`
        )
...
export function isDeepLinkRevision(
    aquaTree: AquaTree,
    revisionHash: string
): boolean | null {
    const revisionData = aquaTree.revisions[revisionHash]
    ...
    if (revisionData) {
        const indexData =
            aquaTree.file_index[revisionData.link_verification_hashes![0]]
        if (indexData) {
            return false
        }
        return true
...
export function fetchLinkedFileName(
    aquaTree: AquaTree,
    revision: Revision
): string {
        ...
        case 'image':
            return files.filter(file => {
                return imageTypes.includes(
                    getFileExtension(file.fileObject[0].fileName).replace(
                        /\s+/g,
                        ''
                    )
                )
...
export function getLatestApiFileInfObject(
    jsonArray: ApiFileInfo[]
): ApiFileInfo | null {
                ...
                for (const anAquaTreeFileObject of allAquaTrees) {
                    ...
                    if (allHashes.includes(hashSigPositionHashString)) {
                        ...
                        signaturePositionCount =
                            getHighestFormIndex(revData) + 1 // sinature count is 0 base
...
export const processContractInformation = (
    selectedFileInfo: ApiFileInfo
    ...
    if (revisionHashes.length > 4) {
        ...
        signatureRevisionHashes = getSignatureRevionHashes(
            fourthItemHashOnwards,
            selectedFileInfo
        )
...
export const processSimpleWorkflowClaim = (
    selectedFileInfo: ApiFileInfo
): ClaimInformation => {
