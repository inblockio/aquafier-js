...
export async function importAquaChain(secondTestPage: Page, context: BrowserContext, shareUrl = ""): Promise<void> {
    ...
    if (shareUrl.length == 0) {
        await secondTestPage.waitForSelector('[data-testid="open-shared-contract-button-0"]', {
            state: 'visible',
            timeout: 10000
        });
    ...
    await secondTestPage.waitForSelector('[data-testid="import-aqua-chain-1-button"]', {
        state: 'visible',
        timeout: 10000
    });
...
export async function registerNewMetaMaskWallet(): Promise<RegisterMetaMaskResponse> {
    ...
    const context = await chromium.launchPersistentContext('userDir/' + crypto.randomUUID().toString(), {
        headless: isCI,
        args: [
...
export async function registerNewMetaMaskWalletAndLogin(): Promise<RegisterMetaMaskResponse> {
    ...
    await testPage.waitForSelector('[data-testid="sign-in-button-navbar-2"]', {state: 'visible'});
    await testPage.click('[data-testid="sign-in-button-navbar-2"]')
...
export async function waitAndClick(testPage: Page, selector: string) {
    await testPage.waitForSelector(selector, {state: 'visible'});
    await testPage.click(selector);
}